...
def gather_nodes(nodes, neighbor_idx):
    ...
    #Repack the features
    neighbors_features = torch.gather(nodes,1,neighbors_flat)
    neighbors_features = neighbors_features.view(list(neighbor_idx.shape)[:3] + [-1])
...
def cat_neighbprs_nodes(h_nodes,h_neighbors, E_idx):
...
class TransformerLayer(nn.Module):
    ...
    def forward(self,h_V,h_E,mask_V = None,mask_attend=None):
        #Parallel computing for the layers
    ...
    def step(self, t, h_V, h_E, mask_V=None, mask_attend=None):
        #Sequentational computating for the layers
...
class MPNNLayer(nn.Module):
    ...
    def forward(self, h_V, h_E, mask_attend= None, mask_V=None):
        #Parallel computing of the layers
...
class NeighborAttention(nn.Module):
    def __init__(self, num_hidden, num_in, num_heads = 4):
        ...
        #Building the layers
        self.W_Q = nn.Linear(num_hidden,num_hidden,bias=False) #Queries
        self.W_K = nn.Linear(num_in, num_hidden, bias=False) #Keys
        self.W_V = nn.Linear(num_in, num_hidden, bias=False) #Values
        self.W_O = nn.Linear(num_hidden,num_hidden, bias=False) #Output
        return
    
    def _masked_softmax(self, attend_logits, mask_attend, dim =-1):
        #Masked softmax function(masked)
    ...
    def step (self, t , h_V, h_E, E_idx, mask_attend=None):
        ...
        #Setting up the Queries,Keys and Values
        ...
        #Scaled inner products with the attention mechanism
